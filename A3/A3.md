...
Some properties is mandatory for the script to execute successfully and provide a key output (e.g., area or a calculated U-value).
Other properties significantly increases the accuracy of the result (e.g., enabling the precise composite U-value calculation for windows) but the script provides a fallback if it is absent.

| Rule | IFC Entity | Attribute/Property Set | Property Name (Example) | Value Type | Constraint |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **01** | `IfcWall` | Any Pset | `ThermalTransmittance` | `IfcMeasureValue` | Must Exist |
| **02** | `IfcSlab`, `IfcRoof` | Any Pset | `U Value` | `IfcMeasureValue` | Must Exist |
| **03** | `IfcWall` | `Pset_WallCommon` | `IsExternal` | `IfcBoolean` | Recommended |
| **04** | Any Opaque Element | `Pset_ElementQuantity` | `NetSideArea` | `IfcAreaMeasure` | Recommended |
| **05** | `IfcWindow` | `Pset_WindowCommon` | `OverallWidth` | `IfcPositiveLengthMeasure` | Must Exist |
| **06** | `IfcWindow` | `Pset_WindowCommon` | `FrameWidth` | `IfcPositiveLengthMeasure` | Recommended (Enables precise area split) |
| **07** | Any Element | `IfcRelAssociatesMaterial` | *Implicit* | | Must Exist (Enables material data extraction) |

### BPMN diagram
The following BPMN diagram shows the process of the inputs for the script.
